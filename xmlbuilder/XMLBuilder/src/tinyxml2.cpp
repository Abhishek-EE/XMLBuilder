       ...
       const int initialLineNum = node->_parseLineNum;
        ...
        if ( ele ) {
            ...
            if ( endTag.Empty() ) {
                ...
                else if ( !XMLUtil::StringEqual( endTag.GetStr(), ele->m_name() ) ) {
                    mismatch = true;
                }
            }
            if ( mismatch ) {
                _document->SetError( XML_ERROR_MISMATCHED_ELEMENT, initialLineNum, "XMLElement name=%s", ele->m_name());
                DeleteNode( node );
                break;
            }
        }
...
const XMLElement* XMLNode::ToElementWithName( const char* name ) const
{
    ...
    if ( XMLUtil::StringEqual( element->m_name(), name ) ) {
       return element;
    }
    return 0;
}
...
const char* XMLAttribute::m_name() const
{
    return _name.GetStr();
}
...
const XMLAttribute* XMLElement::FindAttribute( const char* name ) const
{
    for( XMLAttribute* a = _rootAttribute; a; a = a->_next ) {
        if ( XMLUtil::StringEqual( a->m_name(), name ) ) {
            return a;
        }
    }
    return 0;
...
XMLAttribute* XMLElement::FindOrCreateAttribute( const char* name )
{
    ...
    for( attrib = _rootAttribute;
        ...
        if ( XMLUtil::StringEqual( attrib->m_name(), name ) ) {
            break;
        }
    }
    if ( !attrib ) {
...
void XMLElement::DeleteAttribute( const char* name )
{
    XMLAttribute* prev = 0;
    for( XMLAttribute* a=_rootAttribute; a; a=a->_next ) {
        if ( XMLUtil::StringEqual( name, a->m_name() ) ) {
            if ( prev ) {
                prev->_next = a->_next;
            }
            else {
...
char* XMLElement::ParseAttributes( char* p, int* curLineNumPtr )
{
    ...
    while( p ) {
        p = XMLUtil::SkipWhiteSpace( p, curLineNumPtr );
        if ( !(*p) ) {
            _document->SetError( XML_ERROR_PARSING_ELEMENT, _parseLineNum, "XMLElement name=%s", m_name() );
            return 0;
        }
        ...
        if (XMLUtil::IsNameStartChar( (unsigned char) *p ) ) {
            ...
            if ( !p || Attribute( attrib->m_name() ) ) {
                DeleteAttribute( attrib );
                _document->SetError( XML_ERROR_PARSING_ATTRIBUTE, attrLineNum, "XMLElement name=%s", m_name() );
                return 0;
            }
...
XMLNode* XMLElement::ShallowClone( XMLDocument* doc ) const
{
    ...
    for( const XMLAttribute* a=FirstAttribute(); a; a=a->Next() ) {
        element->SetAttribute( a->m_name(), a->Value() );					// fixme: this will always allocate memory. Intern?
    }
    return element;
}
...
bool XMLElement::ShallowEqual( const XMLNode* compare ) const
{
    ...
    if ( other && XMLUtil::StringEqual( other->m_name(), m_name() )) {
...
XMLPrinter::XMLPrinter( bool file, bool compact, int depth ) :
...
bool XMLPrinter::VisitEnter( const XMLElement& element, const XMLAttribute* attribute )
{
    ...
    OpenElement( element.m_name(), compactMode );
    while ( attribute ) {
        PushAttribute( attribute->m_name(), attribute->Value() );
